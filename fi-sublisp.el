...
the major-mode of the buffer."
  (interactive)
  (fi:eval-send (point-min) (point-max)))
...
correct package, of course."
  ...
  (let* ((stuff (buffer-substring start end))
    ...
    (send-string-load sublisp-process stuff subprocess-map-nl-to-cr)
...
(defun send-region-load (process start end nl-to-cr)
  (send-string-load process (buffer-substring start end) nl-to-cr))
